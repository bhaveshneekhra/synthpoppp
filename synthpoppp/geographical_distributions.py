...
class PopulationDensitySampler:
	...
	def get_lat_long_samples(self, n, polygon):
		...
		sample = subset.sample(weights='population_density', n=(n*10), replace=True).copy()  # Was n* 10 earlier
		
		sample.reset_index(drop=True, inplace=True)
		
		sample['latitude'] = sample['latitude'] + np.random.uniform(-0.005, 0.005, size=sample.shape[0])  # Was -0.015, 0.015
		
		sample['longitude'] = sample['longitude'] + np.random.uniform(-0.005, 0.005, size=sample.shape[0])  # Was -0.015, 0.015
...
class JobsPlacesAddition:
	...
	def assign_workplaces(self, adult_synthetic_population):
		individuals = adult_synthetic_population.sort_values(by=['JobType', 'WorksAtSameCategory']).reset_index(drop=True)
		individuals["WorkplaceID"] = None
		for group in tqdm(individuals.groupby(['JobType', 'WorksAtSameCategory'])):
			current_individuals_set_index = individuals[(individuals['JobType']==group[0][0])&(individuals['WorksAtSameCategory']==group[0][1])].index
			
			if(group[0][1]):
				current_workplaces_set = self.workplaces[self.workplaces['JobType']==group[0][0]].reset_index(drop=True)
			else:
				current_workplaces_set = self.workplaces[self.workplaces['JobType']!=group[0][0]].reset_index(drop=True)
				
			individuals_geocode = individuals.loc[current_individuals_set_index][['H_Lat', 'H_Lon']].values.astype(np.float32)
			workplaces_geocode = current_workplaces_set[['W_Lat', 'W_Lon']].values.astype(np.float32)
			# workplaces_geocode_df = current_workplaces_set[['W_Lat', 'W_Lon']]
			...
			individuals["WorkplaceID"].loc[current_individuals_set_index] = current_workplaces_set["WorkplaceID"].iloc[place_index].values
	...
	def assign_schools(self, child_synthetic_population):
		...
		# schools_geocode_df = self.schools[['School_Lat', 'School_Lon']]
	...
	def assign_public_places(self, synthetic_population):
		...
		# public_places_geocode_df = self.public_places[['PublicPlaceLat', 'PublicPlaceLong']]
	...
	def perform_transforms(self, synthetic_population):
		...
		adults['WorksAtSameCategory'] = True #= np.random.uniform(size=(adults.shape[0],))>0.05
